...
# Load TFLite model
interpreter = tf.lite.Interpreter(model_path="AI\\Models\\autoencoder\\autoencoder_model.tflite")
...
def check_realtime_point(point):
    ...
    # Reconstruct using TFLite model
    # Set input tensor
    interpreter.set_tensor(input_details[0]['index'], point_norm.astype(np.float32))
    interpreter.invoke()
